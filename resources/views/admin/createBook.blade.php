...
@section('content')
    <div class="mx-auto w-full min-h-[100vh] flex">
        ...
        <div class="w-3/4 mx-12 my-8 px-8 py-4  bg-white rounded-lg">
            <h1 class="text-xl font-semibold">Create Books</h1>
            <form action="{{ route('admin.storeBook') }}" method="POST" class="mt-6 space-y-4">
                @csrf
                <div>
                    <label for="judul" class="block text-sm font-medium">Judul</label>
                    <input type="text" id="judul" name="judul" class="w-full border p-2 rounded-md" required>
                </div>
                <div>
                    <label for="code" class="block text-sm font-medium">Code</label>
                    <input type="text" id="code" name="code" class="w-full border p-2 rounded-md" required>
                </div>
                <div>
                    <label for="description" class="block text-sm font-medium">Description</label>
                    <textarea id="description" name="description" class="w-full border p-2 rounded-md" rows="4"></textarea>
                </div>
                <div>
                    <label for="image" class="block text-sm font-medium">Image (URL)</label>
                    <input type="url" id="image" name="image" class="w-full border p-2 rounded-md" required>
                </div>
                <div>
